# Core Infrastructure Module for Campaign Guardian MCP Server
# Includes: Cloud Run service, Secret Manager, Cloud Scheduler, IAM

locals {
  service_name = "campaign-guardian-mcp"
  common_labels = {
...
resource "google_service_account" "runtime" {
  ...
  display_name = "Campaign Guardian Runtime Service Account"
...
resource "google_cloud_scheduler_job" "preflight_check" {
  ...
  name        = "${local.service_name}-preflight"
  description = "Pre-flight campaign checks"
  project     = var.project_id
  region      = var.region
...
resource "google_cloud_scheduler_job" "inflight_monitor" {
  ...
  name        = "${local.service_name}-inflight"
  description = "In-flight campaign monitoring"
  project     = var.project_id
  region      = var.region
