output "uptime_check_ids" {
  description = "IDs of the created per-service (Cloud Run URL) uptime checks"
  value       = { for k, v in google_monitoring_uptime_check_config.health : k => v.uptime_check_id }
}

output "fleet_lb_uptime_check_ids" {
  description = "IDs of the fleet load-balancer uptime checks (empty when fleet_domain is unset)"
  value       = { for k, v in google_monitoring_uptime_check_config.fleet_lb : k => v.uptime_check_id }
}
...
output "alert_policy_ids" {
  ...
  value = merge(
    ...
    { for k, v in google_monitoring_alert_policy.fleet_lb_failure : "${k}-fleet-lb-failure" => v.name },
