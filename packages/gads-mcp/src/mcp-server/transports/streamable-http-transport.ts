...
export function createMcpHttpServer(
  ...
  const storageBackend = config.gcsBucketName
    ? resolveStorageBackend({ gcsBucket: config.gcsBucketName, gcsPrefix: "gads-mcp" })
    : undefined;

  const findingStore = createFindingStore({
    filePath: storageBackend
      ? "findings/gads-mcp-findings.jsonl"
      : join(process.cwd(), "data", "findings", "gads-mcp-findings.jsonl"),
    retentionDays: 30,
    logger,
    storageBackend,
  });
  ...
  // Auth strategy
  // -----------------------------------------------------------------------
  const authStrategy = config.mcpAuthMode === "gads-headers"
    ...
    if (!sessionId) {
      ...
      if (adapter) {
        ...
        if (!config.gadsDeveloperToken || !config.gadsClientId || !config.gadsClientSecret || !config.gadsRefreshToken) {
          return c.json(
            { error: "Google Ads credentials required. Set GADS_DEVELOPER_TOKEN, GADS_CLIENT_ID, GADS_CLIENT_SECRET, and GADS_REFRESH_TOKEN env vars, or use MCP_AUTH_MODE=gads-headers." },
            400
          );
        }
        const { GAdsRefreshTokenAuthAdapter } = await import("../../auth/gads-auth-adapter.js");
        const envAdapter = new GAdsRefreshTokenAuthAdapter({
          developerToken: config.gadsDeveloperToken,
          clientId: config.gadsClientId,
          clientSecret: config.gadsClientSecret,
  ...
  const server = serve(
    ...
    (info) => {
      logger.info(
        ...
        `Google Ads MCP Server listening at http://${info.address}:${info.port}/mcp`
      );
    }
  );
