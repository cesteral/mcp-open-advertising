...
/**
 * `@cesteral/gads-mcp/testkit` — public testing surface for downstream
 * consumers (notably `cesteral-intelligence`'s preview test suite) and
 * upstream conformance tests.
 *
 * Exports:
 * - `GAdsWriteFixture` and related types
 ...
 * - `applyGAdsPatch` — re-exported from the tool's symbolic-apply helper so
 *   consumers can wire their own assertions
 */

import { applyGAdsPatch } from "../mcp-server/tools/utils/dry-run.js";
import type { GAdsEntityKindKey, GAdsOperation, GAdsWriteFixture } from "./types.js";
...
export { applyGAdsPatch } from "../mcp-server/tools/utils/dry-run.js";
...
export function assertContract(
  ...
  const got = applyGAdsPatch(
    fixture.args.entityType,
    fixture.args.customerId,
