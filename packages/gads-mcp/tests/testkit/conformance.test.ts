...
/**
 * Upstream conformance: every fixture in the Google Ads testkit registry must
 ...
 * (`@cesteral/gads-mcp/testkit`) rather than the relative path, to verify the
...
import * as publicTestkit from "@cesteral/gads-mcp/testkit";

describe("gads-mcp testkit conformance", () => {
  ...
  it("ships at least one fixture per round-2 (operation, entityKind) pair", () => {
    ...
    // Governed scope: pause/resume on campaign + adGroup, update_budget on
    // campaignBudget (budget lives on the campaignBudget entity).
    ...
    expect(pairs).toContain("update_budget::campaignBudget");
  });
