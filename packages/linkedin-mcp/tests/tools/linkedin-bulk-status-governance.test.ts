...
const baseInput = {
  entityType: "campaign",
  entityUrns: ["urn:li:sponsoredCampaign:1", "urn:li:sponsoredCampaign:2"],
  status: "PAUSED",
};

describe("linkedin_bulk_update_status governance contract (effect class)", () => {
  ...
  beforeEach(() => {
    ...
    svc = {
      bulkUpdateStatus: vi.fn().mockResolvedValue({
        results: [
          { entityUrn: "urn:li:sponsoredCampaign:1", success: true },
          { entityUrn: "urn:li:sponsoredCampaign:2", success: false, error: "x" },
        ],
      }),
    };
  ...
  it("dry_run returns a symbolic effect preview, no confirmation or API call", async () => {
    ...
    expect(result.dryRun?.expectedEffect).toEqual({
      ...
      summary: { entity_kind: "campaign", requested: 2, target_status: "PAUSED" },
    });
  ...
  it("dry_run flags an empty entity URN", async () => {
    const result = await bulkUpdateStatusLogic(
      { ...baseInput, entityUrns: [""], dry_run: true } as any,
      ctx,
      sdk
    );
    expect(result.dryRun?.wouldSucceed).toBe(false);
    expect(result.dryRun?.validationErrors[0]?.code).toBe("INVALID_ENTITY_URN");
  });
