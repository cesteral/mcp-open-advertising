...
describe("linkedin_adjust_bids governance contract (effect class)", () => {
  ...
  beforeEach(() => {
    ...
    svc = {
      adjustBids: vi.fn().mockResolvedValue({
        results: [{ campaignUrn: "urn:li:sponsoredCampaign:1", success: true }],
      }),
    };
  ...
  it("dry_run returns a symbolic effect preview, no API call or confirmation", async () => {
    const result = await adjustBidsLogic(
      {
        adjustments: [
          { campaignUrn: "urn:li:sponsoredCampaign:1", amount: "12.00", currencyCode: "USD" },
        ],
        dry_run: true,
      } as any,
      ctx,
    ...
    expect(result.dryRun?.expectedEffect).toEqual({
      ...
      summary: { entity_label: "campaign", requested: 1 },
    });
  ...
  it("execute returns the effect identity + null-kind capability", async () => {
    ...
    expect(result.effect).toEqual({
      effectKind: "bids_adjusted",
      summary: { entity_label: "campaign", requested: 1, succeeded: 1, failed: 0 },
    });
  ...
  it("formatter renders a dry-run message without a false success", () => {
    ...
    expect(content[0].text).toContain("Dry run: adjusting bids on 1 campaign(s) would succeed");
