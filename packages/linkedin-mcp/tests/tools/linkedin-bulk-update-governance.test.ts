...
const baseInput = {
  entityType: "campaign",
  items: [
    { entityUrn: "urn:li:sponsoredCampaign:1", data: { dailyBudget: 1 } },
    { entityUrn: "urn:li:sponsoredCampaign:2", data: { dailyBudget: 2 } },
  ],
};

describe("linkedin_bulk_update_entities governance contract (effect class)", () => {
  ...
  it("dry_run flags an empty update payload", async () => {
    const result = await bulkUpdateEntitiesLogic(
      { ...baseInput, items: [{ entityUrn: "urn:1", data: {} }], dry_run: true } as any,
      ctx,
      sdk
    );
