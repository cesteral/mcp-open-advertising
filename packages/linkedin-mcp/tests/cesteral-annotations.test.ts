...
describe("linkedin-mcp cesteral.* annotations (round 3)", () => {
  it("annotates linkedin_update_entity as a write contract with the canonical fields", () => {
    const cesteral = updateEntityTool.annotations?.cesteral;
    expect(cesteral).toBeDefined();
    expect(cesteral!.kind).toBe("write");
    expect(cesteral!.platform).toBe("linkedin_ads");
    expect(cesteral!.contractPlatformSlug).toBe("linkedin_ads");
    expect(cesteral!.contractToolSlug).toBe("update_entity");
    expect(cesteral!.contractId).toBe("linkedin_ads.update_entity.v1");
    ...
    // campaign only — campaignGroup is intentionally out of scope.
    expect(cesteral.entityKinds).toEqual(["campaign"]);
  ...
  it("annotates linkedin_get_entity as a read contract with no operation/readPartner", () => {
    ...
    expect(cesteral!.contractId).toBe("linkedin_ads.get_entity.v1");
    expect(cesteral!.schemaVersion).toBe(1);
    expect(cesteral!.entityKinds).toEqual(["campaign"]);
    expect(cesteral!.entityIdArgs).toEqual(["entityUrn"]);
