...
/**
 * Upstream conformance: every fixture in the CM360 testkit registry must
 ...
 * (`@cesteral/cm360-mcp/testkit`) rather than the relative path, to verify the
...
import * as publicTestkit from "@cesteral/cm360-mcp/testkit";

describe("cm360-mcp testkit conformance", () => {
  ...
  it("ships at least one fixture per round-4 (operation, entityKind) pair", () => {
    ...
    expect(pairs).toContain("update_status::campaign");
    expect(pairs).toContain("pause::ad");
    expect(pairs).toContain("resume::ad");
    expect(pairs).toContain("update_status::ad");
  });
  ...
  it("getFixtures filters by entityKind", () => {
    const ads = getFixtures(undefined, "ad");
    expect(ads.length).toBeGreaterThan(0);
    expect(ads.every((fx) => fx.entityKind === "ad")).toBe(true);
  });
