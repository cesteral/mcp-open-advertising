...
describe("cm360_submit_report governance contract (effect class)", () => {
  let svc: { createReport: ReturnType<typeof vi.fn> };

  beforeEach(() => {
    vi.clearAllMocks();
    svc = {
      createReport: vi.fn().mockResolvedValue({ reportId: "rep-1", fileId: "file-1" }),
    };
  ...
  it("dry_run flags an empty report name", async () => {
    const result = await submitReportLogic(
      { ...baseInput, name: "   ", dry_run: true } as any,
      ctx,
      sdk
    );
  ...
  it("execute returns the effect identity + null-kind capability", async () => {
    ...
    expect(svc.createReport).toHaveBeenCalledOnce();
    expect(result.reportId).toBe("rep-1");
    expect(result.fileId).toBe("file-1");
    expect(result.effect).toEqual({
      effectKind: "report_requested",
      summary: { report_type: "STANDARD", report_handle: "rep-1" },
    });
  ...
  it("formatter renders a dry-run message without a false success", () => {
    ...
    expect(content[0].text).toContain("Dry run: submitting a STANDARD report would succeed");
