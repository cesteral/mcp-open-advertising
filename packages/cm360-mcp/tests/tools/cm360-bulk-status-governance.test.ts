...
describe("cm360_bulk_update_status governance contract (effect class)", () => {
  ...
  it("dry_run returns a symbolic effect preview, no confirmation or API call", async () => {
    ...
    expect(result.dryRun?.expectedEffect).toEqual({
      ...
      summary: { entity_kind: "campaign", requested: 2, target_status: "ARCHIVED" },
    });
  ...
  it("formatter renders a dry-run message without a false success", () => {
    ...
    expect(content[0].text).toContain(
      "Dry run: bulk status change of 2 entity(s) to ARCHIVED would succeed"
    );
