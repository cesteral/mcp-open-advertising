...
describe("cm360_bulk_create_entities governance contract (effect class)", () => {
  ...
  it("dry_run returns a symbolic effect preview, no API call", async () => {
    ...
    expect(result.dryRun?.expectedEffect).toEqual({
      ...
      summary: { entity_kind: "placement", requested: 2 },
    });
  ...
  it("formatter renders a dry-run message without a false success", () => {
    ...
    expect(content[0].text).toContain("Dry run: bulk-creating 2 placement(s) would succeed");
