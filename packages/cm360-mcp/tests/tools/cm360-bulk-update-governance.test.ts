...
const baseInput = {
  ...
  entityType: "campaign",
  items: [
    { entityId: "111", data: { name: "A" } },
    { entityId: "222", data: { name: "B" } },
  ],
};

describe("cm360_bulk_update_entities governance contract (effect class)", () => {
  ...
  beforeEach(() => {
    ...
    svc = {
      bulkUpdateEntities: vi.fn().mockResolvedValue([
        { entityId: "111", success: true, entity: {} },
        { entityId: "222", success: false, error: "x" },
      ]),
    };
    mockResolveSessionServices.mockReturnValue({ cm360Service: svc });
