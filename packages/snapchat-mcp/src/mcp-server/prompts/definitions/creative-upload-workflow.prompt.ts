...
export const snapchatCreativeUploadWorkflowPrompt: Prompt = {
  name: "creative_upload_workflow",
  description: "Step-by-step guide for uploading creative assets and creating Snapchat Ads creatives",
  arguments: [
    {
      name: "adAccountId",
...
export function getSnapchatCreativeUploadWorkflowMessage(args?: Record<string, string>): string {
  ...
  return `# Snapchat Ads Creative Upload Workflow
...
## Overview
Snapchat creative workflow: Upload media → Create Ad Group → Create Ad with creative
...
⚠️ **GOTCHA**: Snapchat ads go through a review process. Initial status should be DISABLE (paused) until ready to launch.
...
| "Ad review rejected" | Policy violation | Review Snapchat ad policies |
