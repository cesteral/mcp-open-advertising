...
const TOOL_NAME = "snapchat_bulk_create_entities";
const TOOL_TITLE = "Snapchat Bulk Create Entities";
const TOOL_DESCRIPTION = `Batch create multiple Snapchat Ads entities of the same type.
...
export const BulkCreateEntitiesInputSchema = z
  .object({
    ...
    dry_run: z
      ...
      .default(false)
      .describe(
        "When true, symbolically validates the batch and returns an EffectDryRunResult under `dryRun` (expected effect = the would-be bulk create) without calling the Snapchat API. No entities are created."
      ),
  })
 ...
 * entity kind). Snapchat has no native bulk validate, so both axes are
...
export const bulkCreateEntitiesTool = {
  ...
  annotations: {
    ...
    cesteral: {
      ...
      entityIdArgs: [],
      schemaVersion: 1,
      contractId: "snapchat.bulk_create_entities.v1",
      supportsDryRun: true,
