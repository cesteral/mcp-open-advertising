...
export const DeleteEntityInputSchema = z
  .object({
    ...
    dry_run: z
      ...
      .default(false)
      .describe(
        "When true, symbolically validates the batch and returns an EffectDryRunResult under `dryRun` (expected effect = the would-be bulk delete) without prompting for confirmation or calling the Snapchat API. No entities are deleted."
      ),
  })
...
export const deleteEntityTool = {
  ...
  annotations: {
    ...
    cesteral: {
      ...
      entityIdArgs: [],
      schemaVersion: 1,
      contractId: "snapchat.delete_entity.v1",
      supportsDryRun: true,
