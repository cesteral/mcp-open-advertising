...
/**
 * Derive a stable filename hint from a Snapchat report download URL. The
 ...
 */
function extractSnapchatReportId(downloadUrl: string): string {
  try {
...
export async function downloadReportLogic(
  ...
  if (result.rawCsv !== undefined) {
    const spill = await spillCsvToGcs({
      ...
      sessionId: sdkContext?.sessionId,
      server: "snapchat",
      reportId: extractSnapchatReportId(input.downloadUrl),
