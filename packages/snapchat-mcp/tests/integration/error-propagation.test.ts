...
const config: any = {
  ...
  mcpLogLevel: "debug",
  otelEnabled: false,
  otelServiceName: "snapchat-mcp-test",
...
describe("mcp transport error propagation (Snapchat)", () => {
  let app: any;
  let shutdown: () => Promise<void>;

  beforeAll(() => {
    mockState.snapchatService.getEntity.mockRejectedValue(
      new McpError(JsonRpcErrorCode.NotFound, "Campaign with ID 999 not found")
    );

    mockState.snapchatService.createEntity.mockRejectedValue(
      new McpError(
        JsonRpcErrorCode.Forbidden,
        "Snapchat API: 403 Forbidden — token missing snapchat-marketing-api scope"
      )
    );

    mockState.snapchatService.listEntities.mockRejectedValue(
  ...
  it("propagates NotFound McpError from get_entity", async () => {
    ...
    expect(mockState.snapchatService.getEntity).toHaveBeenCalledOnce();
