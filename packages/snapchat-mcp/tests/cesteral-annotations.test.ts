...
describe("snapchat-mcp cesteral.* annotations (round 4)", () => {
  it("annotates snapchat_update_entity as a write contract with the canonical fields", () => {
    const cesteral = updateEntityTool.annotations?.cesteral;
    expect(cesteral).toBeDefined();
    expect(cesteral!.kind).toBe("write");
    expect(cesteral!.platform).toBe("snapchat");
    expect(cesteral!.contractPlatformSlug).toBe("snapchat");
    expect(cesteral!.contractToolSlug).toBe("update_entity");
    expect(cesteral!.contractId).toBe("snapchat.update_entity.v1");
    ...
    // campaign + ad_group (Snapchat "ad squad") + ad.
    expect(cesteral.entityKinds).toEqual(["campaign", "ad_group", "ad"]);
    expect(cesteral.entityIdArgs).toEqual(["entityId"]);
    expect(cesteral.readPartner.toolName).toBe("snapchat_get_entity");
    expect(cesteral.readPartner.argMap).toEqual({
      entityType: "entityType",
      adAccountId: "adAccountId",
      entityId: "entityId",
    });
    // R4-U3: symbolic-apply dry-run + before/after capture.
  ...
  it("annotates snapchat_get_entity as a read contract with no operation/readPartner", () => {
    ...
    expect(cesteral!.contractId).toBe("snapchat.get_entity.v1");
    expect(cesteral!.schemaVersion).toBe(1);
    expect(cesteral!.entityKinds).toEqual(["campaign", "ad_group", "ad"]);
    expect(cesteral!.entityIdArgs).toEqual(["entityId"]);
