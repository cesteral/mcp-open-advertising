...
describe("SnapchatAccessTokenAdapter", () => {
  ...
  describe("validate()", () => {
    ...
    it("throws on Snapchat error code", async () => {
      mockFetchWithTimeout.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          code: 40001,
      ...
      await expect(adapter.validate()).rejects.toThrow("Snapchat token validation failed");
    });
    ...
    it("throws on HTTP error", async () => {
      ...
      await expect(adapter.validate()).rejects.toThrow("Snapchat token validation HTTP error");
    });
...
describe("SnapchatRefreshTokenAdapter", () => {
  ...
  describe("getAccessToken", () => {
    it("fetches new token when no cache", async () => {
      ...
      expect(url).toBe(
        "https://business-api.snapchat.com/open_api/v1.3/oauth2/access_token/"
      );
      expect(body).toEqual({
        app_id: "test-app-id",
    ...
    it("error on non-ok HTTP response", async () => {
      ...
      await expect(adapter.getAccessToken()).rejects.toThrow(
        "Snapchat token refresh failed: 401 Unauthorized. Invalid credentials"
      );
    });

    it("error on non-zero Snapchat code", async () => {
      const adapter = new SnapchatRefreshTokenAdapter(
        MOCK_REFRESH_CREDENTIALS,
        "adv-123"
      );
      ...
      await expect(adapter.getAccessToken()).rejects.toThrow(
        "Snapchat token refresh failed: code=40100 message=Unauthorized"
      );
    });
  });
  ...
  describe("validate", () => {
    it("validates token via user info endpoint", async () => {
      ...
      expect(userInfoUrl).toBe(
        "https://business-api.snapchat.com/open_api/v1.3/user/info/"
      );
      expect(
