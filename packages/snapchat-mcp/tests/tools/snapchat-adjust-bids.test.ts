...
describe("snapchat_adjust_bids governance contract (effect class)", () => {
  ...
  it("dry_run returns a symbolic effect preview, no API call or confirmation", async () => {
    const result = await adjustBidsLogic(
      {
        adAccountId: "1",
        adjustments: [{ adGroupId: "ag-1", bidPrice: 1.5 }],
        dry_run: true,
      } as any,
      ctx,
      sdk
    ...
    expect(result.dryRun?.expectedEffect).toEqual({
      ...
      summary: { entity_label: "ad_squad", requested: 1 },
    });
  ...
  it("execute returns the effect identity + null-kind capability", async () => {
    const result = await adjustBidsLogic(
      { adAccountId: "1", adjustments: [{ adGroupId: "ag-1", bidPrice: 1.5 }] } as any,
      ctx,
      sdk
    );
    ...
    expect(result.effect).toEqual({
      effectKind: "bids_adjusted",
      summary: { entity_label: "ad_squad", requested: 1, succeeded: 1, failed: 0 },
    });
