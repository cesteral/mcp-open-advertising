...
vi.mock("@modelcontextprotocol/sdk/server/mcp.js", () => ({
  ...
  ResourceTemplate: vi.fn().mockImplementation(() => ({ kind: "template" })),
}));
...
describe("Snapchat server + transport", () => {
  ...
  it("builds HTTP transport via shared factory", async () => {
    const logger = { info: vi.fn(), warn: vi.fn(), error: vi.fn(), debug: vi.fn() } as any;
    const config = {
      mcpAuthMode: "snapchat-bearer",
      mcpAuthSecretKey: "secret",
      snapchatApiBaseUrl: "https://business-api.snapchat.com",
      snapchatAccessToken: "token",
      snapchatAdvertiserId: "123",
    } as any;
