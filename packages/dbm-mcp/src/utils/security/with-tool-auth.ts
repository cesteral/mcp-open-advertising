...
export interface AuthInfo {
  ...
  token?: string; // Raw JWT (for forwarding to Bid Manager API if needed)
}
 ...
 * Note: This function requires appConfig to be passed or will use a permissive default
 */
export function withRequiredScopes(
  requiredScopes: string[],
  authMode: "none" | "jwt" | "oauth" = "none"
): void {
 ...
 * @param authMode Authentication mode (default: none)
...
export function withToolAuth<TInput, TOutput>(
  ...
  return async (input, context, sdkContext) => {
    ...
    withRequiredScopes(requiredScopes, authMode);
 ...
 */
export function hasAnyScope(scopes: string[], authMode: "none" | "jwt" | "oauth" = "none"): boolean {
  if (authMode === "none") {
    return true;
  }
 ...
 */
export function hasAllScopes(scopes: string[], authMode: "none" | "jwt" | "oauth" = "none"): boolean {
  if (authMode === "none") {
    return true;
  }
