...
const TOOL_NAME = "msads_get_pacing_status";
const TOOL_TITLE = "Calculate Campaign Pacing (Client-Side)";
const TOOL_DESCRIPTION =
  "Client-side pacing calculator — does NOT call the Microsoft Advertising API. Computes actual vs expected delivery given caller-supplied budget, spend-to-date, and flight dates. " +
  "To populate spend, first call \`msads_get_report\`; to populate budget and flight dates, first call \`msads_get_entity\` for the campaign.";
...
export const GetPacingStatusInputSchema = z
  .object({
    accountId: z.string().min(1).describe("Microsoft Advertising Account ID"),
    campaignId: z.string().min(1).describe("Microsoft Advertising Campaign ID to check pacing for"),
