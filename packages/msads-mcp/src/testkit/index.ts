...
/**
 * `@cesteral/msads-mcp/testkit` — public testing surface for downstream
 * consumers (notably `cesteral-intelligence`'s preview test suite) and
 * upstream conformance tests.
 *
 * Exports:
 * - `MsAdsWriteFixture` and related types
 ...
 * - `applyMsAdsPatch` — re-exported from the tool's symbolic-apply helper so
 *   consumers can wire their own assertions
 */

import { applyMsAdsPatch } from "../mcp-server/tools/utils/dry-run.js";
import type { MsAdsEntityKindKey, MsAdsOperation, MsAdsWriteFixture } from "./types.js";
...
export { applyMsAdsPatch } from "../mcp-server/tools/utils/dry-run.js";
