...
describe("msads-mcp cesteral.* annotations (round 4)", () => {
  it("annotates msads_update_entity as a write contract with the canonical fields", () => {
    const cesteral = updateEntityTool.annotations?.cesteral;
    expect(cesteral).toBeDefined();
    expect(cesteral!.kind).toBe("write");
    expect(cesteral!.platform).toBe("msads");
    expect(cesteral!.contractPlatformSlug).toBe("msads");
    expect(cesteral!.contractToolSlug).toBe("update_entity");
    expect(cesteral!.contractId).toBe("msads.update_entity.v1");
    ...
    // campaign + ad_group + ad + campaign_budget.
    expect(cesteral.entityKinds).toEqual(["campaign", "ad_group", "ad", "campaign_budget"]);
    expect(cesteral.entityIdArgs).toEqual(["entityId"]);
    expect(cesteral.readPartner.toolName).toBe("msads_get_entity");
  ...
  it("annotates msads_get_entity as a read contract with no operation/readPartner", () => {
    ...
    expect(cesteral!.contractId).toBe("msads.get_entity.v1");
    expect(cesteral!.schemaVersion).toBe(1);
    expect(cesteral!.entityKinds).toEqual(["campaign", "ad_group", "ad", "campaign_budget"]);
    expect(cesteral!.entityIdArgs).toEqual(["entityId"]);
