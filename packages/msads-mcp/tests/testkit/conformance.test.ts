...
/**
 * Upstream conformance: every fixture in the Microsoft Ads testkit registry
 * must round-trip through `assertContract` cleanly. This locks the
 * symbolic-apply contract against the canonical state-transition fixtures the
 * platform owns.
 *
 * The smoke test at the bottom imports from the published subpath
 * (`@cesteral/msads-mcp/testkit`) rather than the relative path, to verify the
...
import * as publicTestkit from "@cesteral/msads-mcp/testkit";

describe("msads-mcp testkit conformance", () => {
  const fixtures = getFixtures();

  it("ships at least one fixture per round-4 (operation, entityKind) pair", () => {
    const pairs = new Set(fixtures.map((fx) => `${fx.operation}::${fx.entityKind}`));
    expect(pairs).toContain("update_budget::campaign");
    expect(pairs).toContain("update_budget::budget");
    expect(pairs).toContain("pause::campaign");
    expect(pairs).toContain("pause::adGroup");
    expect(pairs).toContain("pause::ad");
    expect(pairs).toContain("resume::campaign");
    expect(pairs).toContain("resume::adGroup");
    expect(pairs).toContain("resume::ad");
  });
