...
describe("msads_bulk_update_entities governance contract (effect class)", () => {
  let svc: {
  ...
  it("dry_run flags an item missing an Id", async () => {
    const result = await bulkUpdateEntitiesLogic(
      { ...baseInput, items: [{ DailyBudget: 100 }], dry_run: true } as any,
      ctx,
      sdk
    );
    expect(result.dryRun?.wouldSucceed).toBe(false);
    expect(result.dryRun?.validationErrors[0]?.code).toBe("MISSING_ID");
  });
