...
const config: any = {
  serviceName: "pinterest-mcp-test",
  port: 3011,
  host: "127.0.0.1",
  nodeEnv: "test",
  ...
  mcpLogLevel: "debug",
  otelEnabled: false,
  otelServiceName: "pinterest-mcp-test",
  otelExporterOtlpTracesEndpoint: undefined,
  otelExporterOtlpMetricsEndpoint: undefined,
  pinterestApiBaseUrl: "https://api.pinterest.com",
  pinterestApiVersion: "v5",
  pinterestRateLimitPerMinute: 100,
};
...
describe("mcp transport error propagation (Pinterest)", () => {
  let app: any;
  let shutdown: () => Promise<void>;

  beforeAll(() => {
    mockState.pinterestService.getEntity.mockRejectedValue(
      new McpError(JsonRpcErrorCode.NotFound, "Campaign with ID 999 not found")
    );

    mockState.pinterestService.createEntity.mockRejectedValue(
      new McpError(
        JsonRpcErrorCode.Forbidden,
        "Pinterest API: 403 Forbidden — token missing ads:write scope"
      )
    );

    mockState.pinterestService.listEntities.mockRejectedValue(
      new McpError(JsonRpcErrorCode.InvalidParams, "adAccountId is required for entityType")
    );
  ...
  it("propagates NotFound McpError from get_entity", async () => {
    ...
    expect(mockState.pinterestService.getEntity).toHaveBeenCalledOnce();
