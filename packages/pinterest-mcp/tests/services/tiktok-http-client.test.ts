...
const BASE_URL = "https://business-api.pinterest.com";
...
describe("PinterestHttpClient", () => {
  ...
  beforeEach(() => {
    client = new PinterestHttpClient(mockAuthAdapter, ADVERTISER_ID, BASE_URL, mockLogger);
  ...
  function mockPinterestSuccessResponse(data: unknown) {
  ...
  function mockPinterestErrorResponse(code: number, message: string) {
    mockFetchWithTimeout.mockResolvedValueOnce({
      ok: true,
      status: 200,
  ...
  describe("GET requests", () => {
    it("injects ad_account_id into query params", async () => {
      mockPinterestSuccessResponse({ list: [], page_info: {} });
    ...
    it("sets Authorization: Bearer header", async () => {
      mockPinterestSuccessResponse({ list: [] });
    ...
    it("returns data field from response on success", async () => {
      ...
      mockPinterestSuccessResponse(mockData);

      const result = await client.get("/open_api/v1.3/campaign/get/");
      expect(result).toEqual(mockData);
    });

    it("throws McpError on Pinterest error code", async () => {
      mockPinterestErrorResponse(40001, "Access token is expired");
  ...
  describe("POST requests", () => {
    it("injects ad_account_id into JSON body", async () => {
      mockPinterestSuccessResponse({ campaign_id: "1800000001" });
  ...
  describe("retry behavior", () => {
    it("retries on rate limit error code 40100", async () => {
      mockPinterestErrorResponse(40100, "Rate limit exceeded");
      mockPinterestSuccessResponse({ list: [] });
    ...
    it("does NOT retry on non-retryable errors (e.g., code 40002)", async () => {
      mockPinterestErrorResponse(40002, "Invalid parameter");
  ...
  describe("error mapping", () => {
    it("maps auth error codes to Unauthorized", async () => {
      mockPinterestErrorResponse(40001, "Access token expired");
    ...
    it("maps rate limit codes to RateLimited", async () => {
      ...
      mockPinterestErrorResponse(40100, "Rate limit");
      mockPinterestErrorResponse(40100, "Rate limit");
      mockPinterestErrorResponse(40100, "Rate limit");
      mockPinterestErrorResponse(40100, "Rate limit");
