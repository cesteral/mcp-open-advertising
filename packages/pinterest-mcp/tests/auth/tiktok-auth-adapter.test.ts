...
describe("PinterestAccessTokenAdapter", () => {
  ...
  describe("validate()", () => {
    ...
    it("throws on Pinterest error code", async () => {
      mockFetchWithTimeout.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          code: 40001,
      ...
      await expect(adapter.validate()).rejects.toThrow("Pinterest token validation failed");
    });
  ...
  describe("validate", () => {
    it("validates token via user info endpoint", async () => {
      ...
      expect(userInfoUrl).toBe(
        "https://business-api.pinterest.com/open_api/v1.3/user/info/"
      );
      expect(
