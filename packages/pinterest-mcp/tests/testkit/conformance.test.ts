...
/**
 * Upstream conformance: every fixture in the Pinterest testkit registry must
 ...
 * (`@cesteral/pinterest-mcp/testkit`) rather than the relative path, to
...
import * as publicTestkit from "@cesteral/pinterest-mcp/testkit";

describe("pinterest-mcp testkit conformance", () => {
  ...
  it("ships at least one fixture per round-4 (operation, entityKind) pair", () => {
    ...
    // update_budget covered for campaign + adGroup (ad has no budget field).
    expect(pairs).toContain("update_budget::campaign");
    expect(pairs).toContain("update_budget::adGroup");
    // pause / resume covered for all three governed entity kinds.
