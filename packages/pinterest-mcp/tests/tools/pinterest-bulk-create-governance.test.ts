...
const baseInput = {
  ...
  items: [{ campaign_name: "A" }, { campaign_name: "B" }],
};

describe("pinterest_bulk_create_entities governance contract (effect class)", () => {
  ...
  it("dry_run flags an empty item object", async () => {
    const result = await bulkCreateEntitiesLogic(
      { ...baseInput, items: [{ campaign_name: "A" }, {}], dry_run: true } as any,
      ctx,
      sdk
    );
  ...
  it("formatter renders a dry-run message without a false success", () => {
    ...
    expect(content[0].text).not.toContain("[0]:");
  });
});
