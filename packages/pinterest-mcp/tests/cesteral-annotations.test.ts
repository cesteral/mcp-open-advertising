...
describe("pinterest-mcp cesteral.* annotations (round 4)", () => {
  it("annotates pinterest_update_entity as a write contract with the canonical fields", () => {
    const cesteral = updateEntityTool.annotations?.cesteral;
    expect(cesteral).toBeDefined();
    expect(cesteral!.kind).toBe("write");
    expect(cesteral!.platform).toBe("pinterest");
    expect(cesteral!.contractPlatformSlug).toBe("pinterest");
    expect(cesteral!.contractToolSlug).toBe("update_entity");
    expect(cesteral!.contractId).toBe("pinterest.update_entity.v1");
    ...
    // campaign + ad_group (adGroup-equivalent) + ad.
    ...
    // R4-U4: symbolic-apply dry-run + before/after capture.
  ...
  it("annotates pinterest_get_entity as a read contract with no operation/readPartner", () => {
    ...
    expect(cesteral!.contractId).toBe("pinterest.get_entity.v1");
    expect(cesteral!.schemaVersion).toBe(1);
    expect(cesteral!.entityKinds).toEqual(["campaign", "ad_group", "ad"]);
    expect(cesteral!.entityIdArgs).toEqual(["entityId"]);
