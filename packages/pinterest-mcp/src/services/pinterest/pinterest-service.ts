...
/** Pinterest page_info response shape */
...
/** Pinterest list response data shape */
interface PinterestListData {
  list: unknown[];
  page_info: PinterestPageInfo;
}

/**
 * Pinterest Service — Generic CRUD operations for Pinterest Marketing API entities,
 * plus bulk operations and entity duplication.
 *
 * Key differences from Meta:
 * - ad_account_id is always required (injected by PinterestHttpClient)
...
export class PinterestService {
  ...
  async listEntities(
    ...
    const result = (await this.httpClient.get(config.listPath, params, context)) as PinterestListData;
  ...
  async createEntity(
    ...
    await this.rateLimiter.consume(`pinterest:default`, 3);
  ...
  async updateEntity(
    ...
    // Pinterest uses POST for updates, with entity ID in body
  ...
  async duplicateEntity(
    ...
    // Pinterest copy endpoints follow pattern /{entity}/copy/
