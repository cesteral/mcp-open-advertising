...
export const BulkUpdateStatusInputSchema = z
  .object({
    ...
    dry_run: z
      ...
      .default(false)
      .describe(
        "When true, symbolically validates the batch and returns an EffectDryRunResult under `dryRun` (expected effect = the would-be bulk status change) without prompting for confirmation or calling the Pinterest API. No statuses are changed."
      ),
  })
...
export const bulkUpdateStatusTool = {
  ...
  annotations: {
    ...
    cesteral: {
      ...
      entityIdArgs: [],
      schemaVersion: 1,
      contractId: "pinterest.bulk_update_status.v1",
      supportsDryRun: true,
