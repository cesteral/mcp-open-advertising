...
export const AdjustBidsInputSchema = z
  .object({
    ...
    dry_run: z
      ...
      .default(false)
      .describe(
        "When true, validates the bid adjustments and returns an EffectDryRunResult under `dryRun` (expected effect = the would-be bid-adjustment batch) without calling the Pinterest API or prompting for confirmation. No bids are changed."
      ),
  })
 ...
 * N ad groups. Pinterest has no native bid validate/preview, so both axes are
...
export const adjustBidsTool = {
  ...
  annotations: {
    ...
    cesteral: {
      ...
      entityIdArgs: [],
      schemaVersion: 1,
      contractId: "pinterest.adjust_bids.v1",
      supportsDryRun: true,
