...
describe("dv360 uploads governance contract (effect class)", () => {
  let dv360Service: { uploadAsset: ReturnType<typeof vi.fn> };

  beforeEach(() => {
    vi.clearAllMocks();
    dv360Service = {
      uploadAsset: vi.fn().mockResolvedValue({ asset: { mediaId: "media-1" } }),
    };
  ...
  for (const c of cases) {
    describe(c.label, () => {
      it("dry_run returns a symbolic effect preview, no download or API call", async () => {
        ...
        expect(dv360Service.uploadAsset).not.toHaveBeenCalled();
      ...
      it("execute returns the effect identity + null-kind capability", async () => {
        ...
        expect(dv360Service.uploadAsset).toHaveBeenCalledOnce();
        expect(result.assetId).toBe("media-1");
      ...
      it("formatter renders a dry-run message without a false success", () => {
        ...
        expect(content[0].text).not.toContain("uploaded to DV360");
      });
    });
  }
});
