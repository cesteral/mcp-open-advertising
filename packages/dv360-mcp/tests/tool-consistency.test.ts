 ...
 * Only matches tool names from table rows within that section to avoid
 * picking up prompt names referenced elsewhere in the text.
 */
function extractToolNamesFromPrompt(promptText: string, prefix: string): string[] {
  ...
  return [
    ...new Set(
      [...summarySection.matchAll(new RegExp(`\`(${prefix}[a-z_]+)\``, "g"))].map(
        (m) => m[1]
      )
    ),
  ];
...
const CONFORMANCE_TOOL_NAMES = new Set(["echo", "test_simple_text", "test_tool_with_logging", "test_elicitation", "test_elicitation_sep1034_defaults", "test_elicitation_sep1330_enums"]);
