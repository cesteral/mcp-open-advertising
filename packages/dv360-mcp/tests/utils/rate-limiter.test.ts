...
describe("RateLimiter", () => {
  ...
  describe("pattern matching", () => {
    it("matches wildcard pattern dv360:* to dv360:123", async () => {
    ...
    it("matches wildcard pattern dv360:* to dv360:abc", async () => {
      limiter.configure("dv360:*", 1, 60_000);

      await limiter.consume("dv360:abc");
      await expect(limiter.consume("dv360:abc")).rejects.toThrow(McpError);
    });
    ...
    it("does NOT match wildcard pattern to non-matching prefix", async () => {
      ...
      // "other:123" should not be limited by "dv360:*"
  ...
  describe("exact match", () => {
    ...
    it("prefers exact match over pattern match", async () => {
      ...
      // "dv360:special" should use the exact match (limit 1)
  ...
  describe("cached regex compilation", () => {
    ...
    it("compiles regex for patterns with wildcards but not for plain strings", async () => {
      // We can verify this indirectly: configure both patterns, then check matching
  ...
  describe("reset and clear", () => {
    ...
    it("clear removes all request timestamps and compiled patterns", async () => {
      limiter.configure("dv360:*", 2, 60_000);
  ...
  describe("cleanup", () => {
    it("removes old timestamps (older than 1 hour) on interval", async () => {
      ...
      // After cleanup runs, the key should be fully available again.
