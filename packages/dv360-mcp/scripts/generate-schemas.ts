...
async function main() {
  ...
  try {
    ...
    const openApiSpec = await convertToOpenAPI(
      schemas,
      discoveryDoc,
      report,
      VALIDATED_CONFIG
    ...
    console.log(`   - Original: ${Math.round(report.sizeAnalysis.originalDiscoverySize / 1024)} KB`);
...
async function generateTypeScriptTypes(specPath: string): Promise<void> {
  const outputPath = path.resolve(
    PACKAGE_ROOT,
    VALIDATED_CONFIG.output.generatedPath,
    'types.ts'
  );
