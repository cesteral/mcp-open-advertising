...
const baseInput = {
  ...
  items: [
    { entityId: "camp456def", data: { CampaignName: "A" } },
    { entityId: "camp789ghi", data: { CampaignName: "B" } },
  ],
};
...
describe("ttd_bulk_update_entities governance contract (effect class)", () => {
  let svc: {
    bulkUpdateEntities: ReturnType<typeof vi.fn>;
  ...
  beforeEach(() => {
    ...
    svc = {
      ...
      bulkUpdateEntities: vi.fn().mockResolvedValue({
        results: [
          { success: true, entity: { Id: "camp456def" } },
  ...
  it("dry_run flags an empty update payload", async () => {
    const result = await bulkUpdateEntitiesLogic(
      { ...baseInput, items: [{ entityId: "x", data: {} }], dry_run: true } as any,
      ctx,
      sdk
    );
  ...
  it("formatter renders a dry-run message without a false success", () => {
    ...
    expect(content[0].text).not.toContain("Bulk update campaign:");
  });
});
