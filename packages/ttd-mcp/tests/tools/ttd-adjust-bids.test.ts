...
describe("ttd_adjust_bids governance contract (effect class)", () => {
        ...
        results: [
          {
            adGroupId: "ag-1",
            success: true,
            entity: { RTBAttributes: { BaseBidCPM: { Amount: 4.5 } } },
          },
        ],
      }),
    };
  ...
  it("dry_run returns a symbolic effect preview, no API call or confirmation", async () => {
    const result = await adjustBidsLogic(
      { adjustments: [{ adGroupId: "ag-1", baseBidCpm: 4.5 }], dry_run: true } as any,
      ctx,
      sdk
    );
  ...
  it("execute returns the effect identity + null-kind capability", async () => {
    const result = await adjustBidsLogic(
      { adjustments: [{ adGroupId: "ag-1", baseBidCpm: 4.5 }] } as any,
      ctx,
      sdk
    );
  ...
  it("formatter renders a dry-run message without a false success", () => {
    ...
    expect(content[0].text).toContain("Dry run: adjusting bids on 1 ad group(s) would succeed");
