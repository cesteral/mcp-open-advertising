...
describe("ttd_submit_report governance contract (effect class)", () => {
  let svc: { createReportSchedule: ReturnType<typeof vi.fn> };
  ...
  beforeEach(() => {
    ...
    mockResolveSessionServices.mockReturnValue({ ttdReportingService: svc });
  });
  ...
  it("dry_run flags a non-positive template id", async () => {
    const result = await submitReportLogic(
      { ...baseInput, reportTemplateId: 0, dry_run: true } as any,
      ctx,
      sdk
    );
  ...
  it("execute returns the effect identity + null-kind capability", async () => {
    ...
    expect(result.effect).toEqual({
      effectKind: "report_requested",
      summary: { report_type: "templated", report_handle: "sched-1" },
    });
  ...
  it("formatter renders a dry-run message without a false success", () => {
    ...
    expect(content[0].text).toContain("Dry run: submitting a templated report would succeed");
