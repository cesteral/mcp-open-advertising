...
describe("ttd-mcp cesteral.* annotations (round 3)", () => {
  it("annotates ttd_update_entity as a write contract with the canonical fields", () => {
    const cesteral = updateEntityTool.annotations?.cesteral;
    expect(cesteral).toBeDefined();
    expect(cesteral!.kind).toBe("write");
    expect(cesteral!.platform).toBe("ttd");
    expect(cesteral!.contractPlatformSlug).toBe("ttd");
    expect(cesteral!.contractToolSlug).toBe("update_entity");
    expect(cesteral!.contractId).toBe("ttd.update_entity.v1");
    ...
    // campaign + ad_group.
    expect(cesteral.entityKinds).toEqual(["campaign", "ad_group"]);
  ...
  it("annotates ttd_get_entity as a read contract with no operation/readPartner", () => {
    ...
    expect(cesteral!.contractId).toBe("ttd.get_entity.v1");
    expect(cesteral!.schemaVersion).toBe(1);
    expect(cesteral!.entityKinds).toEqual(["campaign", "ad_group"]);
    expect(cesteral!.entityIdArgs).toEqual(["entityId"]);
