...
const TOOL_NAME = "ttd_get_pacing_status";
const TOOL_TITLE = "Calculate Campaign Pacing (Client-Side)";
const TOOL_DESCRIPTION =
  "Client-side pacing calculator — does NOT call The Trade Desk's API. Computes actual vs expected delivery given caller-supplied budget, spend-to-date, and flight dates. " +
  "To populate spend, first call \`ttd_get_report\`; to populate budget and flight dates, first call \`ttd_get_entity\` for the campaign.";
...
export const GetPacingStatusInputSchema = z
  .object({
    advertiserId: z.string().min(1).describe("The Trade Desk Advertiser ID"),
    campaignId: z.string().min(1).describe("The Trade Desk Campaign ID to check pacing for"),
