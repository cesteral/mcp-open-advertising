...
export async function listReportTemplatesLogic(
  ...
  throwIfGraphqlErrors(raw, "GraphQL error listing report templates", {
  ...
  const hasNextPage = pageInfo.hasNextPage as boolean | undefined;
  const endCursor = pageInfo.endCursor as string | undefined;

  return {
    templates: nodes,
    pagination: buildPaginationOutput({
      nextCursor: hasNextPage === true ? (endCursor ?? null) : null,
  ...
  return [
    {
      type: "text" as const,
      text:
        `Found ${pageSize} report template(s)` +
        (totalCount !== undefined ? ` (${totalCount} total)` : "") +
        `:\n\n${JSON.stringify(result.templates, null, 2)}${formatPaginationHint(result.pagination)}\n\nTimestamp: ${result.timestamp}`,
    },
  ];
}
