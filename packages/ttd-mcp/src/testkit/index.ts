...
/**
 * `@cesteral/ttd-mcp/testkit` — public testing surface for downstream
 * consumers (notably `cesteral-intelligence`'s preview test suite) and
 * upstream conformance tests.
 *
 * Exports:
 * - `TtdWriteFixture` and related types
 ...
 * - `applyTtdPatch` — re-exported from the tool's symbolic-apply helper so
 *   consumers can wire their own assertions
 */

import { applyTtdPatch } from "../mcp-server/tools/utils/dry-run.js";
