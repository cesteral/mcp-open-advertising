...
describe("GCS spill integration", () => {
  ...
  it("returns spill metadata when the helper reports success (CSV)", async () => {
    process.env.REPORT_SPILL_BUCKET = "test-bucket";
    mockDownloadReport.mockResolvedValueOnce({
      headers: ["date"],
      rows: [["2026-03-01"]],
  ...
  it("threads application/json mimeType through to the spill helper and metadata", async () => {
    process.env.REPORT_SPILL_BUCKET = "test-bucket";
    mockDownloadReport.mockResolvedValueOnce({
      headers: ["advertiser"],
      rows: [["adv-1"]],
    ...
    mockSpillCsvToGcs.mockResolvedValueOnce({
      ...
      bytes: 42,
      rowCount: 1,
      signedUrl: "https://signed.example/report.json",
      expiresAt: "2030-01-01T00:00:00.000Z",
