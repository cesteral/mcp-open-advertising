/**
 * AmazonDsp Entity Schema Resources
 */
...
const ENTITY_SCHEMA_CONTENT: Record<AmazonDspEntityType, string> = {
  campaign: `# AmazonDsp Campaign Fields
...
## Read-Only Fields
campaign_id, created_time, modify_time, profile_id
`,

  adGroup: `# AmazonDsp Ad Group Fields
...
| Field | Type | Description |
|-------|------|-------------|
| video_id | string | Video ID from AmazonDsp Creative Library |
...
`,
  ...
  creative: `# AmazonDsp Creative Fields
...
## Read-Only Fields
creative_id, profile_id, created_time
`,
};

function buildEntitySchemaMarkdown(entityType: AmazonDspEntityType): string {
  return ENTITY_SCHEMA_CONTENT[entityType] ?? `# AmazonDsp ${entityType}\n\nNo schema information available.\n`;
}
...
export const entitySchemaResources: Resource[] = getSupportedEntityTypes().map((entityType) => ({
  ...
  name: `AmazonDsp ${entityType} Schema`,
  description: `Field reference for AmazonDsp ${entityType} entity including required fields, optional fields, and read-only fields`,
...
export const entitySchemaAllResource: Resource = {
  ...
  name: "AmazonDsp All Entity Schemas",
  description: "Combined field reference for all AmazonDsp Ads entity types",
  mimeType: "text/markdown",
