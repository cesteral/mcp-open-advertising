...
const TOOL_NAME = "amazon_dsp_get_audience_estimate";
const TOOL_TITLE = "AmazonDsp Audience Size Estimate";
const TOOL_DESCRIPTION = `Get an estimated audience size for a AmazonDsp targeting configuration.
...
export const GetAudienceEstimateInputSchema = z
  ...
  .describe("Parameters for getting a AmazonDsp audience size estimate");

export const GetAudienceEstimateOutputSchema = z
  .object({
    estimate: z.record(z.any()).describe("Audience size estimate data from AmazonDsp"),
...
export async function getAudienceEstimateLogic(
  ...
  const estimate = await amazonDspService.getAudienceEstimate(
