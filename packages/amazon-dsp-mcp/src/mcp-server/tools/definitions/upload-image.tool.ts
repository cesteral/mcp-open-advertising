...
const TOOL_NAME = "amazon_dsp_upload_image";
const TOOL_TITLE = "Upload Image to AmazonDsp Ads";
const TOOL_DESCRIPTION = `Upload an image to AmazonDsp Ads Library from a URL.

The server downloads the image and uploads it to AmazonDsp's ad image library.
...
}).describe("Parameters for uploading an image to AmazonDsp");
...
export async function uploadImageLogic(
  ...
  const result = await amazonDspService.client.postMultipart(
    ...
    contentType,
    context
  ) as AmazonDspImageUploadResponse;

  const imageId = result.image_id;
  if (!imageId) {
    throw new Error("AmazonDsp image upload failed: no image_id returned");
  }
...
export const uploadImageTool = {
  ...
  inputExamples: [
    {
      label: "Upload a AmazonDsp ad image",
      input: {
