...
export const BulkUpdateEntitiesInputSchema = z
  .object({
    ...
    dry_run: z
      ...
      .default(false)
      .describe(
        "When true, symbolically validates the batch and returns an EffectDryRunResult under `dryRun` (expected effect = the would-be bulk update) without prompting for confirmation or calling the Amazon DSP API. No entities are updated."
      ),
  })
...
export async function bulkUpdateEntitiesLogic(
  ...
  const confirmed = await elicitBulkMutationConfirmation({
    ...
    summary: "Applying field updates across multiple Amazon DSP entities.",
...
export const bulkUpdateEntitiesTool = {
  ...
  annotations: {
    ...
    cesteral: {
      ...
      entityIdArgs: [],
      schemaVersion: 1,
      contractId: "amazon_dsp.bulk_update_entities.v1",
      supportsDryRun: true,
