...
export const BulkUpdateStatusInputSchema = z
  .object({
    ...
    dry_run: z
      ...
      .default(false)
      .describe(
        "When true, symbolically validates the batch and returns an EffectDryRunResult under `dryRun` (expected effect = the would-be bulk status change) without prompting for confirmation or calling the Amazon DSP API. No statuses are changed."
      ),
  })
 ...
 * status change to one target status). Amazon DSP has no native bulk validate,
...
export const bulkUpdateStatusTool = {
  ...
  annotations: {
    ...
    cesteral: {
      ...
      entityIdArgs: [],
      schemaVersion: 1,
      contractId: "amazon_dsp.bulk_update_status.v1",
      supportsDryRun: true,
