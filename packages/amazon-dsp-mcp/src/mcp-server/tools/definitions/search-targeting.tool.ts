...
const TOOL_TITLE = "AmazonDsp Search Targeting Options";
const TOOL_DESCRIPTION = `Search AmazonDsp targeting options by keyword or browse by type.

**Common targeting types:**
- INTEREST_CATEGORY — Interest and hobby categories
- BEHAVIOR — User behavior segments
- HASHTAG — AmazonDsp hashtag interests
...
export const SearchTargetingInputSchema = z
  ...
  .describe("Parameters for searching AmazonDsp targeting options");
...
export async function searchTargetingLogic(
  ...
  const results = (await amazonDspService.searchTargeting(
    input.targetingType,
    input.query,
    input.limit,
    context
