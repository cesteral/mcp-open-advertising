...
const TOOL_TITLE = "Upload Video to AmazonDsp Ads";
const TOOL_DESCRIPTION = `Upload a video to AmazonDsp Ads Library from a URL.

The server downloads the video and uploads it to AmazonDsp's ad video library.
...
export const UploadVideoInputSchema = z.object({
  profileId: z.string().describe("AmazonDsp Advertiser ID"),
...
}).describe("Parameters for uploading a video to AmazonDsp");
...
interface AmazonDspVideoInfoResponse {
  list?: AmazonDspVideoInfoItem[];
}
...
export async function uploadVideoLogic(
  ...
  const uploadResult = await amazonDspService.client.postMultipart(
    ...
    contentType,
    context
  ) as AmazonDspVideoUploadResponse;

  const videoId = uploadResult.video_id;
  if (!videoId) {
    throw new Error("AmazonDsp video upload failed: no video_id returned");
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const statusResult = await amazonDspService.client.post(
      "/open_api/v1.3/file/video/ad/info/",
      { video_ids: [videoId] },
      context
    ) as AmazonDspVideoInfoResponse;
    ...
    if (videoStatus === "error" || videoStatus === "deleted") {
      throw new Error(`AmazonDsp video processing failed: status=${videoStatus}`);
    }
  }
...
export const uploadVideoTool = {
  ...
  inputExamples: [
    {
      label: "Upload a AmazonDsp campaign video",
      input: {
