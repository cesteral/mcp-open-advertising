...
const TOOL_NAME = "amazon_dsp_get_pacing_status";
const TOOL_TITLE = "Calculate Campaign Pacing (Client-Side)";
const TOOL_DESCRIPTION =
  "Client-side pacing calculator — does NOT call the Amazon DSP API. Computes actual vs expected delivery given caller-supplied budget, spend-to-date, and flight dates. " +
  "To populate spend, first call \`amazon_dsp_get_report\`; to populate budget and flight dates, first call \`amazon_dsp_get_entity\` for the campaign.";
...
export const GetPacingStatusInputSchema = z
  .object({
    advertiserId: z.string().min(1).describe("Amazon DSP Advertiser ID"),
    campaignId: z.string().min(1).describe("Amazon DSP campaign (order) ID to check pacing for"),
...
> = {
  ...
  inputExamples: [
    ...
    },
    {
      ...
      input: {
        advertiserId: "9876543210",
        campaignId: "6122334455",
        spendToDate: 4200,
        budgetTotal: 10000,
