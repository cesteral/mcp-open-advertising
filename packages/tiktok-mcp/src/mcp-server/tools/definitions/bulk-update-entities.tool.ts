...
const TOOL_NAME = "tiktok_bulk_update_entities";
const TOOL_TITLE = "TikTok Bulk Update Entities";
const TOOL_DESCRIPTION = `Batch update multiple TikTok Ads entities of the same type.

**Supported entity types:** ${getEntityTypeEnum().join(", ")}

Each item must include an \`entityId\` and a \`data\` object with fields to update.
Updates are applied concurrently (max concurrency 5). advertiser_id is automatically injected.
...
export const BulkUpdateEntitiesInputSchema = z
  .object({
    ...
    dry_run: z
      ...
      .default(false)
      .describe(
        "When true, symbolically validates the batch and returns an EffectDryRunResult under `dryRun` (expected effect = the would-be bulk update) without prompting for confirmation or calling the TikTok API. No entities are updated."
      ),
  })
...
export async function bulkUpdateEntitiesLogic(
  ...
  const confirmed = await elicitBulkMutationConfirmation({
    ...
    summary: "Applying field updates across multiple TikTok Ads entities.",
 ...
 * projects the would-be effect (an N-item update of one entity kind). TikTok has
...
export const bulkUpdateEntitiesTool = {
  ...
  annotations: {
    ...
    cesteral: {
      ...
      entityIdArgs: [],
      schemaVersion: 1,
      contractId: "tiktok.bulk_update_entities.v1",
      supportsDryRun: true,
