...
export const BulkCreateEntitiesInputSchema = z
  .object({
    ...
    dry_run: z
      ...
      .default(false)
      .describe(
        "When true, symbolically validates the batch and returns an EffectDryRunResult under `dryRun` (expected effect = the would-be bulk create) without calling the TikTok API. No entities are created."
      ),
  })
...
export const bulkCreateEntitiesTool = {
  ...
  annotations: {
    ...
    cesteral: {
      ...
      entityIdArgs: [],
      schemaVersion: 1,
      contractId: "tiktok.bulk_create_entities.v1",
      supportsDryRun: true,
