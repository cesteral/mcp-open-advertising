...
describe("tiktok_bulk_update_status governance contract (effect class)", () => {
    ...
    svc = {
      bulkUpdateStatus: vi.fn().mockResolvedValue({
        results: [
          ...
          { entityId: "1800222222222", success: false, error: "x" },
        ],
      }),
    };
  ...
  it("dry_run returns a symbolic effect preview, no confirmation or API call", async () => {
    ...
    expect(result.dryRun?.expectedEffect).toEqual({
      ...
      summary: { entity_kind: "campaign", requested: 2, target_status: "DISABLE" },
    });
  ...
  it("formatter renders a dry-run message without a false success", () => {
    ...
    expect(content[0].text).toContain(
      "Dry run: bulk status change of 2 entity(s) to DISABLE would succeed"
    );
