...
describe("tiktok-mcp cesteral.* annotations (round 3)", () => {
  it("annotates tiktok_update_entity as a write contract with the canonical fields", () => {
    const cesteral = updateEntityTool.annotations?.cesteral;
    expect(cesteral).toBeDefined();
    expect(cesteral!.kind).toBe("write");
    expect(cesteral!.platform).toBe("tiktok");
    expect(cesteral!.contractPlatformSlug).toBe("tiktok");
    expect(cesteral!.contractToolSlug).toBe("update_entity");
    expect(cesteral!.contractId).toBe("tiktok.update_entity.v1");
    ...
    // campaign + ad_group (ad-group-equivalent) + ad.
    ...
    // R3-U4: symbolic-apply dry-run + before/after capture.
  ...
  it("annotates tiktok_get_entity as a read contract with no operation/readPartner", () => {
    ...
    expect(cesteral!.contractId).toBe("tiktok.get_entity.v1");
    expect(cesteral!.schemaVersion).toBe(1);
    expect(cesteral!.entityKinds).toEqual(["campaign", "ad_group", "ad"]);
    expect(cesteral!.entityIdArgs).toEqual(["entityId"]);
