...
describe("Meta capabilities overview", () => {
  const registeredNames = allTools
    .map((tool) => tool.name)
    .filter((name) => !CONFORMANCE_TOOL_NAMES.has(name));
  const capabilitiesResource = allResources.find(
    (resource) => resource.uri === "server-capabilities://meta-mcp/overview"
  );

  it("registers the Meta capabilities overview resource", () => {
    expect(capabilitiesResource).toBeDefined();
  });

  it("groups every registered Meta tool for progressive discovery", () => {
