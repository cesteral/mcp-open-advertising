...
const baseInput = {
  entityType: "adSet",
  items: [
    { entityId: "1", data: { daily_budget: 100 } },
    { entityId: "2", data: { daily_budget: 200 } },
  ],
};

describe("meta_bulk_update_entities governance contract (effect class)", () => {
    ...
    svc = {
      bulkUpdateEntities: vi.fn().mockResolvedValue({
        results: [
          ...
          { entityId: "2", success: false, error: "x" },
        ],
      }),
    };
  ...
  it("dry_run returns a symbolic effect preview, no confirmation or API call", async () => {
    ...
    expect(result.dryRun?.expectedEffect).toEqual({
      ...
      summary: { entity_kind: "adSet", requested: 2 },
    });
  ...
  it("dry_run flags an empty update payload", async () => {
    const result = await bulkUpdateEntitiesLogic(
      { ...baseInput, items: [{ entityId: "1", data: {} }], dry_run: true } as any,
      ctx,
      sdk
    );
  ...
  it("formatter renders a dry-run message without a false success", () => {
    ...
    expect(content[0].text).toContain("Dry run: bulk-updating 2 adSet(s) would succeed");
