...
describe("mcp transport CRUD integration", () => {
  ...
  beforeAll(() => {
    ...
    mockState.metaService.createEntity.mockImplementation(
      async (_entityType: string, _adAccountId: string, data: Record<string, unknown>) => {
        const id = "23456789012345";
        const entity = { id, ...data };
        mockState.entities.set(id, entity);
        return entity;
      }
    );
    mockState.metaService.getEntity.mockImplementation(
    ...
    mockState.metaService.updateEntity.mockImplementation(
      async (entityId: string, data: Record<string, unknown>) => {
        const existing = mockState.entities.get(entityId) ?? {};
        const updated = { ...existing, ...data, id: entityId };
        mockState.entities.set(entityId, updated);
        return { success: true };
      }
    );
    mockState.metaService.deleteEntity.mockImplementation(async (entityId: string) => {
  ...
  it("supports create -> get -> update -> delete through /mcp", async () => {
    ...
    expect(mockState.entities.has("23456789012345")).toBe(false);
  });
});
