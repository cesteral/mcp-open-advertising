...
function assertJsonCompatible(value: unknown): unknown {
  ...
  if (value !== null && t === "object") {
    ...
    if (Array.isArray(value)) value.forEach(assertJsonCompatible);
    else for (const v of Object.values(value as Record<string, unknown>)) assertJsonCompatible(v);
  }
  return value;
}
```
...
```json
{
  ...
  "vectors": [
    { "input": { "b": 1, "a": 2 }, "actionHash": "<FILL_BY_RUNNING_TEST>" },
    { "input": { "customerId": "1", "entityId": "2", "data": { "status": "PAUSED" } }, "actionHash": "<FILL>" },
    { "input": { "list": [ { "y": 1, "x": 2 } ], "z": null }, "actionHash": "<FILL>" },
    { "input": { "unicode": "naïve—café", "n": 9007199254740991 }, "actionHash": "<FILL>" },
    { "input": {}, "actionHash": "<FILL>" },
    { "input": { "empty": [], "obj": {} }, "actionHash": "<FILL>" }
  ]
}
```
...
import { hashActionInput } from "../src/index.js";
...
**Step 3: Generate hashes** — run once, read FAIL output, paste actual hashes into the JSON `actionHash` fields. Re-run → PASS. (This is the self-pinning step; once governance vectors arrive, replace the file and flip `source`.)
...
**Step 4: Run-pass.** (All existing `update_entity` annotations across servers will now fail to typecheck until Task adds `writeClass: "entity"` + `executableArgsExclude` — that is PR 2's job; in PR 1 only the shared types + a couple of shared fixtures change. To keep PR 1 green without touching 13 servers, build only shared+contract-hash in PR 1 and defer server typecheck to PR 2. Document this in the commit.)

> **Note:** If you prefer PR 1 to keep the whole repo green, fold the 13 `update_entity` annotation updates (add `writeClass: "entity"`, `executableArgsExclude: ["dry_run"]`) into PR 1 as Task 5b. Recommended, since it is mechanical and keeps `pnpm run typecheck` clean. Treat 5b as: for each of the 13 `update-entity.tool.ts`, add the two fields, run that package's typecheck, commit per package or in one sweep.
...
**Step 2: Run-fail. Step 3: Implement** `FirestoreJtiStore` using `@google-cloud/firestore` `create()` (throws on existing doc → `"replayed"`), TTL via doc field + Firestore TTL policy (documented). `selectJtiStore` factory + enforce-in-memory warning.
...
- accepted under previous secret → `OK`
- expired → `EXPIRED`
...
- **jti NOT consumed when a prior check fails** (inject a jti store spy; assert `consumeOnce` not called on mismatch)
...
export interface DecisionTokenVerdict {
  ok: boolean;
  reasonCode: string;           // OK | MISSING_TOKEN | UNSUPPORTED_ALG | ...
  claims?: { sub?: string; contractId?: string; jti?: string; approvalId?: string };
}
export async function verifyDecisionToken(opts: VerifyOpts): Promise<DecisionTokenVerdict> { /* order per design §4 */ }
```
- Use `jose.jwtVerify` with `algorithms: ["HS256"]` (rejects `none`/others → catch → `UNSUPPORTED_ALG` vs `INVALID_SIGNATURE` by inspecting the protected header `alg` first via `jose.decodeProtectedHeader`).
- Try `current` then `previous` secret.
- Claim checks in the exact order from design §4; **`jtiStore.consumeOnce` is the LAST step**, only reached when everything else is `OK`.
...
**Step 1: Failing test** — `logDecisionTokenVerdict(logger, { verdict, mode, contractId, toolName, claims })` emits a structured record with `component: "governance-audit"`, fields `sub`, `contractId`, `toolName`, `jti`, `status`, `reasonCode`, `mode`, and **never includes the raw token/secret** (assert serialized log has no `token` field). `recordDecisionTokenVerification(reasonCode, mode)` increments a counter (spy the OTEL meter like `metrics.test.ts` does).
...
**Steps:**
1. `pnpm run build` — expect clean (if Task 5b deferred, server typecheck may fail; either do 5b or scope build to `--filter @cesteral/shared --filter @cesteral/contract-hash`).
