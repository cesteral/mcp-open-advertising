...
```json
{
  ...
  "vectors": [
    {
      "input": { "b": 1, "a": 2 },
      "stableString": "<FILL_BY_RUNNING_TEST>",
      "actionHash": "<FILL>"
    },
    {
    ...
    {
      "input": { "list": [{ "y": 1, "x": 2 }], "z": null },
      "stableString": "<FILL>",
      "actionHash": "<FILL>"
    },
    {
      "input": { "unicode": "naïve—café", "n": 9007199254740991 },
      "stableString": "<FILL>",
      "actionHash": "<FILL>"
    },
...
it("dispatchedCapability accepts null canonicalEntityKind for effect writes", () => {
  expect(
    DispatchedCapabilitySchema.parse({ operation: "upload", canonicalEntityKind: null })
      .canonicalEntityKind
  ).toBeNull();
  expect(
    DispatchedCapabilitySchema.parse({ operation: "update", canonicalEntityKind: "campaign" })
      .canonicalEntityKind
  ).toBe("campaign");
});
...
it("carries an optional typed decisionToken", () => {
  ...
  expect(runWithRequestContext(ctx, async () => getRequestContext()?.decisionToken)).resolves.toBe(
    "eyJ..."
  );
});
```
...
export async function verifyDecisionToken(opts: VerifyOpts): Promise<DecisionTokenVerdict> {
  /* order below */
}
```
...
if (cesteral?.kind === "write") {
  ...
  if (mode !== "off") {
    ...
    logDecisionTokenVerdict(auditLogger, {
      verdict,
      mode,
      contractId: cesteral.contractId,
      toolName: tool.name,
...
### Rollout status (entity-write families) — updated 2026-06-04
...
| `delete_entity` / `remove_entity` | merged — **fleet-wide**. Single-entity = entity-class delete (meta/gads/dv360/linkedin/ttd); bulk = effect-class `bulk_job` (amazon-dsp/msads/pinterest/snapchat/tiktok); cm360 = effect-class `manage` (only `floodlightActivity`, non-canonical) | #47 (testkit), #52 (oos dry-run fix), #64 (5 bulk), #65 (ttd + cm360)                      |
| `duplicate_entity`                | merged — meta/linkedin/amazon-dsp/pinterest/tiktok/dv360                                                                                                                                                                                           | #53                                                                                        |
| `bulk_create_entities`            | merged — **fleet-wide** (11 platforms); effect-class `bulk_job`                                                                                                                                                                                    | #60                                                                                        |
| `bulk_update_entities`            | merged — effect-class `bulk_job` (10 platforms)                                                                                                                                                                                                    | #62                                                                                        |
| `bulk_update_status`              | merged — effect-class `bulk_job` (+ gads `bulk_mutate`); 12 tools                                                                                                                                                                                  | #63                                                                                        |

### Rollout status (effect-write families) — updated 2026-06-05

Template-B effect-write families (all warn-eligible; none flipped to `enforce`):

| Family                                   | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| media uploads / `submit_report`          | merged (#57, #56)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| report-schedule / template CRUD          | merged (#59, #61)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| SA360 conversion uploads / `adjust_bids` | merged (#61, #55)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **TTD long tail**                        | merged — clears the TTD allowlist (now `[]`). 8 tools: graphql passthrough (`graphql_query` — `manage`, no dry-run as it is an arbitrary query/mutation passthrough), graphql bulk (`graphql_query_bulk`/`graphql_mutation_bulk` — `bulk_job`), `graphql_cancel_bulk_job`/`manage_bid_list`/`manage_seed` (`manage`), `bulk_manage_bid_lists` (`bulk_job`), `archive_entities` (`archive`). All effect-class; symbolic dry-run everywhere except `graphql_query`. Effect summaries carry audit identity only (counts/ids/job status/query fingerprint) — never the raw query/mutation/data payloads. |
...
6. **`readPartner.argMap` self-containment — RESOLVED (2026-06-05).** Originally deferred from #53: the DV360 governed tools mapped only `advertiserId` + the typed IDs and **omitted the `entityType` discriminator** that `dv360_get_entity` requires. The hardening PR delivered all three parts: (a) added `entityType: "entityType"` to `readPartner.argMap` across DV360 `update`/`delete`/`duplicate`/`create` together; (b) added a release-gate assertion (`scripts/lib/readpartner-argmap.test.mjs`) that boots each server and, over the real `tools/list` wire, asserts every governed write's `argMap` covers all **required** read-partner args the write tool can supply from its own inputs (response-derived ids like create's `entityId` are correctly exempt); (c) the gate's fleet-wide sweep surfaced that the **entire fleet** omitted the `entityType` mapping (not just DV360) — `entityType` was added to all 28 governed entity-write argMaps (every `create`/`update`/`delete`/`duplicate`/`remove` across the 11 platforms). **definitionHash delta:** `argMap` is in the annotation hash projection, so these 28 tools' `definitionHash` changes — same re-attestation coordination as Risk #1.
