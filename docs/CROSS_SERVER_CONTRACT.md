...
- **Targeting discovery** — `get_targeting_options` (linkedin, meta, msads, pinterest, snapchat, tiktok) · `list_targeting_options` (cm360). CM360's tool is a discovery/list read like its siblings; it follows the DCM/CM360 API's `*targeting*` list vocabulary. The name is a **frozen wire identifier** (renaming it would change the published `definitionHash`), so the variant is documented here rather than renamed. **gads deliberately ships no dedicated targeting-discovery tool** — Google Ads exposes its targeting criteria as first-class **queryable resources** (`geo_target_constant`, `language_constant`, `carrier_constant`, `mobile_device_constant`, `operating_system_version_constant`, `topic_constant`, `user_interest`, `detailed_demographic`, `life_event`, …), so `gads_gaql_search` **is** the intended discovery path and is strictly more capable than a fixed-shape options tool (e.g. `SELECT geo_target_constant.id, geo_target_constant.name, geo_target_constant.country_code FROM geo_target_constant WHERE geo_target_constant.name = 'Paris'`). It pages via `pageToken` like every other GAQL read. A bespoke `gads_get_targeting_options` shim would only re-expose a subset of these resources behind a new frozen wire identifier / `definitionHash` for no capability gain, so the GAQL escape hatch is the documented path rather than a gap. Enumeration code must not expect a `gads_*targeting*` tool. The other servers without the read follow the same rule: **ttd, amazon-dsp, sa360, and dbm ship no targeting-discovery tool at all**, and **dv360 exposes only targeting management** — its `*_assigned_targeting` tools (get/list/create/delete/validate) operate on targeting already assigned to a line item, not a browse of the available option catalog. So "targeting discovery" is genuinely a six-server capability (linkedin, meta, msads, pinterest, snapchat, tiktok) plus cm360's list variant, and enumeration code must not treat its absence elsewhere as a gap.
