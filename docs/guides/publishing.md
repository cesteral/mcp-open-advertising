...
**Publisher:** `scripts/publish-all.sh` publishes each package from its directory with a single `pnpm publish --access public --no-git-checks [--provenance]`. `pnpm publish` packs on the fly, rewriting `"workspace:*"` dependency ranges to the resolved versions _inside_ the uploaded tarball (a plain `npm publish` of a source directory would ship the literal `workspace:*` string and break consumers), and forwards `--provenance` to the underlying `npm publish` so CI runs attach the build attestation. Historical note: pnpm 8.15 (pinned until June 2026) had no provenance support, which forced a two-step `pnpm pack` + `npm publish <tarball>` workaround — collapsed after the pnpm 10 migration per `docs/plans/2026-06-10-pnpm-provenance-followup.md`.
